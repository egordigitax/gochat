...
func (m *PGChatsStorage) GetChatByUid(chatUid string) (types.Chat, error) {
	var chat types.Chat
	query := `
    SELECT 
        uc.id,
...
func (m *PGChatsStorage) GetChatsByUserUid(userUid string, limit, offset int) ([]types.Chat, error) {
...
func (m *PGChatsStorage) FetchChatsLastMessages(chats *[]types.Chat) error {
	if len(*chats) == 0 {
	...
	messages := make(map[string]types.Message, len(*chats))
